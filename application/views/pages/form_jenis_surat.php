                ...
                <div class="box box-primary">
                    <div class="box-body">
                        ...
                        <div class="col-md-6" style="margin-top: 10px; padding: 0 5px 0 0;">
                            <?php echo form_button('ResetBtn', 'Reset', ' class="btn btn-default btn-flat btn-block" onclick="resetBtn1()"')?>
                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        table = $('#jenis_surat').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('admin/data_jenis_surat'); ?>",
                "type": "POST"
            },
    ...
    function editJenisSurat(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('admin/edit_jenis_surat/') ?>"+id,
            type: "GET",
            dataType: "JSON",
            ...
            if(confirm('Hapus '+list_id.length+' data?'))
            {
                $.ajax({
                    ...
                    success: function(data)
                    {
                        if(data.status)
                        {
                            $('[name="ajaxDelBtn"]').attr('disabled', true);
                            table.ajax.reload(null,false);
                        }
                        else
                        {
